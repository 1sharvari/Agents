...
- **Trigger**: Executed by OrchestratorAgent when Jira tickets exist in **In Review** (`codeReview`) status.
...
2. **PAUSE AND HALT EXECUTION**:
   > *Automated code review completed for `<ticketKey>`. Please perform second round of human review on the Pull Request and transition ticket to **'QA Ready'** (`qaReady`) in Jira to authorize QA automation testing.*
3. **DO NOT PROCEED** until human approval moves the ticket to **QA Ready** (`qaReady`).
